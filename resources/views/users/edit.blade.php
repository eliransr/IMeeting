                    ...
                    <div class="card-body">
                        <form method="post" action="{{ route('user.update', $user) }}" autocomplete="off">
                            ...
                            <div class="pl-lg-4">
                                ...
                                <select class="form-control m-bot15" name="role" id="role" class="form-control form-control-alternative{{ $errors->has('role') ? ' is-invalid' : '' }}" placeholder="{{ __('Role') }}" value="{{ old('role', $user->role) }}" required>
                                @foreach($roles as $role)   
                                    <option value="{{ $role->role }}" {{ $selectedRole = $role->id ? 'selected="selected"' : '' }}>{{ $role->role }}</option> 
                                @endforeach
                                </select>
